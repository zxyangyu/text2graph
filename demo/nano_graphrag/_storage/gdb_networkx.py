...
from .._utils import logger, convert_graph_to_json
from ..base import (
    BaseGraphStorage,
...
class NetworkXStorage(BaseGraphStorage):
    ...
    @staticmethod
    def write_nx_graph_to_json(graph: nx.Graph, file_name):
        logger.info(
            f"Writing graph with {graph.number_of_nodes()} nodes, {graph.number_of_edges()} edges"
        )
        json_data=convert_graph_to_json(graph)
    ...
    def __post_init__(self):
        ...
        self._json_file= os.path.join(
            self.global_config["working_dir"], f"graph.json"
        )
    ...
    async def index_done_callback(self):
        ...
        NetworkXStorage.write_nx_graph_to_json(self._graph, self._json_file)
