...
from ._op_heck import _build_local_query_context_heck
...
async def extract_entities_to_json(
...
async def _find_most_related_community_from_entities(
    ...
    ] #获取level<=query_param.level的cluster
    related_community_keys_counts = dict(Counter(related_community_dup_keys)) # 计算出现次数
...
async def _build_local_query_context(
    ...
    use_communities = await _find_most_related_community_from_entities(  #在这步之前运行community search 代码，同时修改graph内node的聚类结果并重新重新生成community report
    ...
    logger.info(
        "entities :"+str([r["entity_name"] for r in results])+"\n"+"communities : "+str([r["title"] for r in use_communities])+"\n"
    )
...
async def local_query(
    ...
    if query_param.community_search_method== 'cd':
        context,use_communities = await _build_local_query_context(
            query,
        ...
        context,use_communities = await _build_local_query_context_heck(
            query,
            knowledge_graph_inst,
            entities_vdb,
            community_reports,
    ...
    return response,context,use_communities
